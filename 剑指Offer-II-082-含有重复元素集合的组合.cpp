...
private:
    void backtrack(vector<vector<int>>& res, vector<int>& cur, vector<int>& candidates, int target, int index) {
        ...
        backtrack(res, cur, candidates, target, i);
        cur.push_back(candidates[index]);
        backtrack(res, cur, candidates, target - candidates[index], index + 1);
        cur.pop_back();
    }
};
...
int main() {
    ...
    cin >> target;
    vector<vector<int>> res = Solution().combinationSum2(nums, target);
